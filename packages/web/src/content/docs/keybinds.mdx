...
Agent-Core has a list of keybinds that you can customize through the Agent-Core config.
...
Agent-Core uses a `leader` key for most keybinds. This avoids conflicts in your terminal.
...
The Agent-Core desktop app prompt input supports common Readline/Emacs-style shortcuts for editing text. These are built-in and currently not configurable via `agent-core.json`.
