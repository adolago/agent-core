...
To debug any issues with Agent-Core, you can check the logs or the session data
...
- **Windows**: `%USERPROFILE%\.local\share\agent-core\log\`
...
agent-core stores session data and other application data on disk at:

- **macOS/Linux**: `~/.local/share/agent-core/`
- **Windows**: `%USERPROFILE%\.local\share\agent-core`
...
If you're experiencing issues with Agent-Core:

1. **Report issues on GitHub**

   The best way to report bugs or request features is through our GitHub repository:

   [**github.com/anomalyco/agent-core/issues**](https://github.com/anomalyco/agent-core/issues)
...
If you encounter API call errors, this may be due to outdated provider packages. agent-core dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
2. Restart agent-core to reinstall the latest provider packages

This will force agent-core to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
agent-core will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
