---
title: Server
description: Interact with agent-core server over HTTP.
---

export const typesUrl = "#"

The `agent-core serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an agent-core client can use.
...
```bash
agent-core serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
agent-core serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Server auth is enabled by default. Set `AGENT_CORE_SERVER_PASSWORD` (or `OPENCODE_SERVER_PASSWORD`) to protect the server with HTTP basic auth. The username defaults to `agent-core`, or set `AGENT_CORE_SERVER_USERNAME` (or `OPENCODE_SERVER_USERNAME`) to override it. Set `AGENT_CORE_DISABLE_SERVER_AUTH=1` to opt out.

```bash
AGENT_CORE_SERVER_PASSWORD=your-password agent-core serve
```
...
When you run `agent-core` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the agent-core server to interact with agent-core programmatically.
:::

This architecture lets agent-core support multiple clients and allows you to interact with agent-core programmatically.

You can run `agent-core serve` to start a standalone server. If you have the
agent-core TUI running, `agent-core serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Agent-Core [IDE](/docs/ide) plugins.
...
The agent-core server exposes the following APIs.
