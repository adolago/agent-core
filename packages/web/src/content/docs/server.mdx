...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `agent-core`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `agent-core serve` and `agent-core web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password agent-core serve
```
