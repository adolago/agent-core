...
You can add external tools to Agent-Core using the _Model Context Protocol_, or MCP. Agent-Core supports both local and remote servers.
...
You can define MCP servers in your [Agent-Core Config](/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="agent-core.jsonc" {6}
{
  "$schema": "agent-core",
  "mcp": {
    "name-of-mcp-server": {
...
Organizations can provide default MCP servers via their `.well-known/agent-core` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
```jsonc title="agent-core.jsonc" {15}
{
  "$schema": "agent-core",
  "mcp": {
    "my-local-mcp-server": {
...
Agent-Core automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Agent-Core will:
...
If the server requires authentication, Agent-Core will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `agent-core mcp auth <server-name>`.
...
```json title="agent-core.json" {7-11}
{
  "$schema": "agent-core",
  "mcp": {
    "my-oauth-server": {
...
```bash
agent-core mcp auth my-oauth-server
```
...
```bash
agent-core mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, Agent-Core will store the tokens securely in `~/.local/share/agent-core/mcp-auth.json`.
...
agent-core mcp debug my-oauth-server
```
...
Your MCPs are available as tools in Agent-Core, alongside built-in tools. So you can manage them through the Agent-Core config like any other tool.
...
```json title="agent-core.json" {11, 14-18}
{
  "$schema": "agent-core",
  "mcp": {
    "my-mcp": {
...
```json title="agent-core.json" {4-8}
{
  "$schema": "agent-core",
  "mcp": {
    "sentry": {
...
```bash
agent-core mcp auth sentry
```

This will open a browser window to complete the OAuth flow and connect Agent-Core to your Sentry account.
...
```json title="agent-core.json" {4-7}
{
  "$schema": "agent-core",
  "mcp": {
    "context7": {
...
```json title="agent-core.json" {7-9}
{
  "$schema": "agent-core",
  "mcp": {
    "context7": {
