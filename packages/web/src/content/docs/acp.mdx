---
title: ACP Support
description: Use Agent-Core in any ACP-compatible editor.
---

Agent-Core supports the [Agent Client Protocol](https://agentclientprotocol.com) or (ACP), allowing you to use it directly in compatible editors and IDEs.
...
To use Agent-Core via ACP, configure your editor to run the `agent-core acp` command.

The command starts Agent-Core as an ACP-compatible subprocess that communicates with your editor over JSON-RPC via stdio.
...
```json title="acp.json"
{
  "agent_servers": {
    "Agent-Core": {
      "command": "/absolute/path/bin/agent-core",
      "args": ["acp"]
    }
  }
}
...
To open it, use the new 'Agent-Core' agent in the AI Chat agent selector.
...
To use Agent-Core as an ACP agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), add the following to your Neovim config:
...
This config sets up CodeCompanion to use Agent-Core as the ACP agent for chat.
...
Agent-Core works the same via ACP as it does in the terminal. All features are supported:
...
- MCP servers configured in your Agent-Core config
