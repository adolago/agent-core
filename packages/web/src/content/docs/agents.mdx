...
There are two types of agents in Agent-Core; primary agents and subagents.
...
Agent-Core comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Agent-Core comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

Agent-Core comes with two built-in primary agents and two built-in subagents.
...
If no temperature is specified, Agent-Core uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Agent-Core config and the project specific config.
...
The model ID in your Agent-Core config uses the format `provider/model-id`. For example, if you're using [Agent-Core Zen](/docs/zen), you would use `agent-core/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/anomalyco/agent-core).
:::
