...
Most popular providers are preloaded by default. If you've added the credentials for a provider through the `/connect` command, they'll be available when you start Agent-Core.
...
Here are several models that work well with Agent-Core, in no particular order. (This is not an exhaustive list nor is it necessarily up to date):
...
Here the full ID is `provider_id/model_id`. For example, if you're using [Agent-Core Zen](/docs/zen), you would use `agent-core/gpt-5.1-codex` for GPT 5.1 Codex.
...
```jsonc title="agent-core.jsonc" {7-12,19-24}
{
  "$schema": "agent-core",
  "provider": {
    "openai": {
...
```jsonc title="agent-core.jsonc" {6-21}
{
  "$schema": "agent-core",
  "provider": {
    "agent-core": {
...
Many models support multiple variants with different configurations. Agent-Core ships with built-in default variants for popular providers.

### Built-in variants

Agent-Core ships with default variants for many providers:
...
```jsonc title="agent-core.jsonc" {7-18}
{
  "$schema": "agent-core",
  "provider": {
    "openai": {
...
When Agent-Core starts up, it checks for models in the following priority order:

1. The `--model` or `-m` command line flag. The format is the same as in the config file: `provider_id/model_id`.

2. The model list in the Agent-Core config.
