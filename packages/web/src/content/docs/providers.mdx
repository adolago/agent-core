...
Agent-Core uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Agent-Core config.
...
in `~/.local/share/agent-core/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your Agent-Core
config.
...
Agent-Core Zen is a list of models provided by the Agent-Core team that have been
tested and verified to work well with Agent-Core. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Agent-Core Zen.
:::

1. Run the `/connect` command in the TUI, select agent-core, and head to [/auth](/auth).
...
To use Amazon Bedrock with Agent-Core:
...
2. **Configure authentication** using one of the following methods:
   ...
   Set one of these environment variables while running agent-core:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY agent-core

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile agent-core

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX agent-core
   ```
   ...
   For project-specific or persistent configuration, use `agent-core.json`:
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for agent-core to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX agent-core
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX agent-core
   ```
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your agent-core config.
...
to store token in agent-core auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
Agent-Core uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Agent-Core
to only use your own GitLab-hosted instance, add the following to your
`agent-core.json` file. It is also recommended to disable session sharing.
...
More documentation is available from your GitLab instance.

##### Configuration

Customize through `agent-core.json`:
...
To use your GitHub Copilot subscription with agent-core:
...
To use Google Vertex AI with Agent-Core:
...
2. Set the required environment variables:
   ...
   Set them while running agent-core.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id agent-core
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through agent-core, you can always configure it yourself.
...
```jsonc title="~/.config/agent-core/agent-core.jsonc"
{
  "$schema": "agent-core",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Install a session plugin and add it to your config.

```json title="agent-core.json"
{
  "plugin": ["<helicone-session-plugin>"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Agent-Core conversation listed as a separate session.
...
You can configure agent-core to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="agent-core.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "agent-core",
  "provider": {
    "llama.cpp": {
...
You can configure agent-core to use local models through LM Studio.

```json title="agent-core.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "agent-core",
  "provider": {
    "lmstudio": {
...
You can configure agent-core to use local models through Ollama.

```json title="agent-core.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "agent-core",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with Agent-Core:
...
Agent-Core Zen is a list of tested and verified models provided by the Agent-Core team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Agent-Core Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Agent-Core Zen**.

   ```txt
   /connect
   ```

3. Enter your Agent-Core API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your agent-core config.
...
5. You can also customize them through your agent-core config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' agent-core
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group agent-core
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Agent-Core:
...
You can also customize models through your agent-core config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with agent-core. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in agent-core.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `agent-core.json` file in your project directory:

   ```json title="agent-core.json" ""myprovider"" {5-15}
   {
     "$schema": "agent-core",
     "provider": {
       "myprovider": {
...
```json title="agent-core.json" {9,11,17-20}
{
  "$schema": "agent-core",
  "provider": {
    "myprovider": {
...
The `limit` fields allow Agent-Core to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `agent-core auth list` to see if the credentials
...
2. For custom providers, check the agent-core config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your agent-core config.
