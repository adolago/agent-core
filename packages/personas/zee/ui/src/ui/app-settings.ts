...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") await loadPresence(host as unknown as ZeeApp);
  if (host.tab === "sessions") await loadSessions(host as unknown as ZeeApp);
  if (host.tab === "cron") await loadCron(host);
  if (host.tab === "skills") await loadSkills(host as unknown as ZeeApp);
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as ZeeApp);
    await loadDevices(host as unknown as ZeeApp);
    await loadConfig(host as unknown as ZeeApp);
    await loadExecApprovals(host as unknown as ZeeApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as ZeeApp);
    await loadConfig(host as unknown as ZeeApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as ZeeApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as ZeeApp, { reset: true });
    scheduleLogsScroll(
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as ZeeApp, false),
    loadPresence(host as unknown as ZeeApp),
    loadSessions(host as unknown as ZeeApp),
    loadCronStatus(host as unknown as ZeeApp),
    loadDebug(host as unknown as ZeeApp),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as ZeeApp, true),
    loadConfigSchema(host as unknown as ZeeApp),
    loadConfig(host as unknown as ZeeApp),
  ]);
}
...
export async function loadCron(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as ZeeApp, false),
    loadCronStatus(host as unknown as ZeeApp),
    loadCronJobs(host as unknown as ZeeApp),
  ]);
}
