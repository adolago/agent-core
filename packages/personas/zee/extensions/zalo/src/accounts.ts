import type { ZeeConfig } from "zee/plugin-sdk";
...
export function listZaloAccountIds(cfg: ZeeConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultZaloAccountId(cfg: ZeeConfig): string {
...
function mergeZaloAccountConfig(cfg: ZeeConfig, accountId: string): ZaloAccountConfig {
...
export function listEnabledZaloAccounts(cfg: ZeeConfig): ResolvedZaloAccount[] {
