...
Zee extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
zee channels login --channel zalouser
...
```bash
zee message send --channel zalouser --target <threadId> --message "Hello from Zee!"
```
...
```bash
zee channels login --channel zalouser              # Login via QR
zee channels login --channel zalouser --account work
zee channels status --probe
zee channels logout --channel zalouser
```
...
```bash
# Text
zee message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
zee message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `zee channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
zee channels login --channel zalouser --account work
zee message send --channel zalouser --account work --target <id> --message "Hello"
