...
export function listBlueBubblesAccountIds(cfg: ZeeConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultBlueBubblesAccountId(cfg: ZeeConfig): string {
...
export function listEnabledBlueBubblesAccounts(cfg: ZeeConfig): ResolvedBlueBubblesAccount[] {
