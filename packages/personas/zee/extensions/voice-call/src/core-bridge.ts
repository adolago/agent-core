...
function resolveZeeRoot(): string {
  if (coreRootCache) return coreRootCache;
  const override =
    process.env.ZEE_ROOT?.trim() ||
    process.env.MOLTBOT_ROOT?.trim() ||
    process.env.CLAWDBOT_ROOT?.trim();
  if (override) {
  ...
  for (const start of candidates) {
    for (const name of ["zee", "moltbot", "clawdbot"]) {
  ...
  throw new Error(
    "Unable to resolve core root. Set ZEE_ROOT (legacy: MOLTBOT_ROOT/CLAWDBOT_ROOT) to the package root.",
  );
}

async function importCoreModule<T>(relativePath: string): Promise<T> {
  const root = resolveZeeRoot();
