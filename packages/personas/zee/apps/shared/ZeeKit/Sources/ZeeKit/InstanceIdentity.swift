...
public enum InstanceIdentity {
    private static let suiteName = "bot.zee.shared"
    private static let legacySuiteNames = ["bot.molt.shared", "com.clawdbot.shared"]
    ...
    private static var legacyDefaults: [UserDefaults] {
        legacySuiteNames.compactMap { UserDefaults(suiteName: $0) }
    }
    ...
    public static let instanceId: String = {
        ...
        for legacyDefaults in Self.legacyDefaults {
            if let legacy = legacyDefaults.string(forKey: instanceIdKey)?
                .trimmingCharacters(in: .whitespacesAndNewlines),
                !legacy.isEmpty
            {
