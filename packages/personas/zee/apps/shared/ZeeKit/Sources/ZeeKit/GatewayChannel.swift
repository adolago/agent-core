...
public actor GatewayChannelActor {
    ...
    private func sendConnect() async throws {
        ...
        let clientName = (options.clientDisplayName ?? InstanceIdentity.displayName)
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let resolvedClientName = clientName.isEmpty ? InstanceIdentity.displayName : clientName

        let reqId = UUID().uuidString
        var client: [String: ProtoAnyCodable] = [
            "name": ProtoAnyCodable(resolvedClientName),
