...
actor PortGuardian {
    ...
    private let logger = Logger(subsystem: "bot.zee", category: "portguard")
    ...
    private func isExpected(_ listener: Listener, port: Int, mode: AppState.ConnectionMode) -> Bool {
        ...
        case .remote:
            ...
            return false
        case .local:
            // The gateway daemon may listen as `zee` or as its runtime (`node`, `bun`, etc).
            if full.contains("gateway-daemon") { return true }
            // If args are unavailable, treat a zee listener as expected.
            if cmd.contains("zee"), full == cmd { return true }
            return false
        case .unconfigured:
            return false
        }
