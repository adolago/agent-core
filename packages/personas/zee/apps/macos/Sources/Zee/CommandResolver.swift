...
enum CommandResolver {
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let zeePaths = self.zeeManagedPaths(home: home)
        if !zeePaths.isEmpty {
            extras.insert(contentsOf: zeePaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + zeePaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func zeeManagedPaths(home: URL) -> [String] {
        let bases = [".zee", ".moltbot", ".clawdbot"]
        var paths: [String] = []
        var seen = Set<String>()
        for baseName in bases {
            let base = home.appendingPathComponent(baseName)
            let bin = base.appendingPathComponent("bin")
            let nodeBin = base.appendingPathComponent("tools/node/bin")
            for candidate in [bin, nodeBin] {
                let path = candidate.path
                if FileManager().fileExists(atPath: path), seen.insert(path).inserted {
                    paths.append(path)
                }
            }
        }
    ...
    {
        let root = configRoot ?? ZeeConfigFile.loadDict()
