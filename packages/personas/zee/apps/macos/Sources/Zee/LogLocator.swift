...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["ZEE_LOG_DIR"], !override.isEmpty {
            return URL(fileURLWithPath: override)
        }

        return URL(fileURLWithPath: "/tmp/zee")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("zee-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("zee-gateway.log")
    }
