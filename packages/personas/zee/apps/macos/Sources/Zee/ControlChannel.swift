...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: ZeeProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "bot.zee", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: ZeeKit.AnyCodable]()) {
                $0[$1.key] = ZeeKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: ZeeProtocol.AnyCodable?) -> [String: ZeeProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: ZeeProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: ZeeKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: ZeeProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
