...
actor GatewayEndpointStore {
    ...
    private static let staticLogger = Logger(subsystem: "com.zee", category: "gateway-endpoint")
    ...
    init(deps: Deps = .live) {
        ...
        let bind = GatewayEndpointStore.resolveGatewayBindMode(
            ...
            env: ProcessInfo.processInfo.environment)
        let customBindHost = GatewayEndpointStore.resolveGatewayCustomBindHost(root: ZeeConfigFile.loadDict())
    ...
    private static func resolveGatewayScheme(
        ...
        env: [String: String]) -> String
    {
        if let envValue = env["ZEE_GATEWAY_TLS"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !envValue.isEmpty
        {
