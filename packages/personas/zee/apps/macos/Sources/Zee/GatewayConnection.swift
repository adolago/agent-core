...
private let gatewayConnectionLogger = Logger(subsystem: "bot.zee", category: "gateway.connection")
...
actor GatewayConnection {
    ...
    private func sessionDefaultString(_ defaults: [String: ZeeProtocol.AnyCodable]?, key: String) -> String {
...
extension GatewayConnection {
    ...
    func sessionsPreview(
        ...
        limit: Int? = nil,
        maxChars: Int? = nil,
        timeoutMs: Int? = nil) async throws -> ZeeSessionsPreviewPayload
    {
        let resolvedKeys = keys
            .map { self.canonicalizeSessionKey($0) }
            .filter { !$0.isEmpty }
        if resolvedKeys.isEmpty {
            return ZeeSessionsPreviewPayload(ts: 0, previews: [])
        }
