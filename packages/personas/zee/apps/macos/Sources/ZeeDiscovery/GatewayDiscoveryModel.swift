...
public final class GatewayDiscoveryModel {
    ...
    private let logger = Logger(subsystem: "bot.zee", category: "gateway-discovery")
    ...
    public func start() {
        ...
        for domain in ZeeBonjour.gatewayServiceDomains {
            let params = NWParameters.tcp
            params.includePeerToPeer = true
            let browser = NWBrowser(
                for: .bonjour(type: ZeeBonjour.gatewayServiceType, domain: domain),
                using: params)
            ...
            browser.start(queue: DispatchQueue(label: "bot.zee.macos.gateway-discovery.\(domain)"))
        }
    ...
    public func refreshWideAreaFallbackNow(timeoutSeconds: TimeInterval = 5.0) {
        let domain = ZeeBonjour.wideAreaGatewayServiceDomain
    ...
    private func updateGateways(for domain: String) {
        ...
        if domain == ZeeBonjour.wideAreaGatewayServiceDomain,
