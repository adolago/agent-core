...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func prefersZeeBinary() async throws {
        ...
        let cmd = CommandResolver.zeeCommand(subcommand: "gateway", defaults: defaults, configRoot: [:])
    ...
    @Test func fallsBackToPnpm() async throws {
        ...
        let cmd = CommandResolver.zeeCommand(subcommand: "rpc", defaults: defaults, configRoot: [:])
    ...
    @Test func buildsSSHCommandForRemoteMode() async throws {
        ...
        defaults.set("zee@example.com:2222", forKey: remoteTargetKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "zee@example.com")
        } else {
            #expect(Bool(false))
        }
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/zee'"))
