...
# Build and bundle Zee into a minimal .app we can open.
# Outputs to dist/Zee.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Zee.app"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/zee/zee/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Zee/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :ZeeBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :ZeeGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Zee"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Zee"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Zee"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Zee" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Zee/Resources/Zee.icns" "$APP_ROOT/Contents/Resources/Zee.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Zee/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
MOLTBOTKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/ZeeKit_ZeeKit.bundle"
if [ -d "$MOLTBOTKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/ZeeKit_ZeeKit.bundle"
  cp -R "$MOLTBOTKIT_BUNDLE" "$APP_ROOT/Contents/Resources/ZeeKit_ZeeKit.bundle"
else
  echo "WARN: ZeeKit resource bundle not found at $MOLTBOTKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Zee"
killall -q Zee 2>/dev/null || true
