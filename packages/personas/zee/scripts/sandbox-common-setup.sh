...
BASE_IMAGE="${BASE_IMAGE:-zeebot-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-zeebot-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=zeebot.sandbox=1)
NOTE
