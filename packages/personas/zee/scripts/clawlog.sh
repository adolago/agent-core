...
show_usage() {
    cat << EOF
clawlog - Zee Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View Zee logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    Zee logs flow through the macOS unified log (subsystem: bot.molt).
