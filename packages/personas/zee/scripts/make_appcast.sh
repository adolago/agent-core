...
ZIP=${1:?"Usage: $0 Zee-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/zee/zee/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Zee-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/zee/zee/releases/download/v${VERSION}/"}
