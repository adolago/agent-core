...
ZIP=${1:?"Usage: $0 Zeebot-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/zeebot/zeebot/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Zeebot-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/zeebot/zeebot/releases/download/v${VERSION}/"}
