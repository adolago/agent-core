...
describe("sendCommand", () => {
  ...
  it("allows media-only sends", async () => {
    const deps = makeDeps({
      sendMessageSlack: vi
        .fn()
        .mockResolvedValue({ messageId: "s2", channelId: "C124" }),
    });
    await sendCommand(
      {
        to: "channel:C124",
  ...
  it("rejects missing message and media", async () => {
    const deps = makeDeps();
    await expect(
      sendCommand(
        {
    ...
    ).rejects.toThrow(/Message body is required/i);
  });
});
