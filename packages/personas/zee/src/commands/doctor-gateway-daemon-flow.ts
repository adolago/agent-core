...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (process.platform === "darwin" && params.cfg.gateway?.mode !== "remote") {
    ...
    await maybeRepairLaunchAgentBootstrap({
      env: { ...process.env, ZEE_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel() },
      title: "Node",
      runtime: params.runtime,
      prompter: params.prompter,
    });
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.ZEE_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("zee gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
