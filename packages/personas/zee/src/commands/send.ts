...
export async function sendCommand(
  ...
  const media = opts.media?.trim();

  if (!message.trim() && !media) {
    throw new Error("Message body is required unless --media is provided.");
  }

  if (opts.dryRun) {
    runtime.log(
      `[dry-run] would send via ${provider} -> ${opts.to}: ${message}${media ? ` (media ${media})` : ""}`,
    );
    return;
  }
    ...
    const results = await withProgress(
      ...
      async () =>
        await deliverOutboundPayloads({
          ...
          payloads: [{ text: message, mediaUrl: media }],
          deps: {
