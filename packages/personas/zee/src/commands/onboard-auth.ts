...
import type { OAuthCredentials, OAuthProvider } from "../agents/llm-types.js";
...
const MAX_OAUTH_PROFILE_PART_LENGTH = 120;

function normalizeProfileIdPart(value: string): string {
  ...
  const sanitized = trimmed
    ...
    .slice(0, MAX_OAUTH_PROFILE_PART_LENGTH);
  return sanitized || "default";
}

export function buildOAuthProfileId(
  provider: OAuthProvider,
  email?: string,
): string {
  return `${provider}:${normalizeProfileIdPart(email ?? "default")}`;
}

function buildPlaceholderApiKey(providerId: string): string {
  const normalized = providerId
    .trim()
    .replace(/[^a-zA-Z0-9_-]/g, "-")
    .slice(0, 60);
  return `placeholder-${normalized || "provider"}`;
}
...
export async function writeOAuthCredentials(
  ...
  const profileId = buildOAuthProfileId(provider, creds.email);
...
export function applyMinimaxProviderConfig(cfg: ZeeConfig): ZeeConfig {
  ...
  if (!providers.lmstudio) {
    providers.lmstudio = {
      ...
      // Placeholder key; real credentials should come from auth profiles or env.
      apiKey: buildPlaceholderApiKey("lmstudio"),
...
export function applyMinimaxHostedProviderConfig(
  ...
  if (!providers.minimax) {
    providers.minimax = {
      ...
      // Placeholder key; real credentials should come from auth profiles or env.
      apiKey: buildPlaceholderApiKey("minimax"),
