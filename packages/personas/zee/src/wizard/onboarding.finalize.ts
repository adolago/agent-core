...
export async function finalizeOnboardingWizard(options: FinalizeOnboardingOptions) {
  ...
  if (!opts.skipHealth) {
    ...
    try {
      ...
      await prompter.note(
        [
          "Docs:",
          "https://docs.zee.bot/gateway/health",
          "https://docs.zee.bot/gateway/troubleshooting",
        ].join("\n"),
        "Health check help",
      );
    }
  ...
  await prompter.note(
    [
      ...
      gatewayStatusLine,
      "Docs: https://docs.zee.bot/web/control-ui",
    ]
      .filter(Boolean)
      .join("\n"),
    "Control UI",
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: ~/.zee/zee.json (gateway.auth.token) or ZEE_GATEWAY_TOKEN.",
        "Web UI stores a copy in this browser's localStorage (zee.control.settings.v1).",
  ...
  await prompter.note(
    ["Back up your agent workspace.", "Docs: https://docs.zee.bot/concepts/agent-workspace"].join(
      "\n",
    ),
    "Workspace backup",
  );
  ...
  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.zee.bot/security",
    "Security",
  );
  ...
  await prompter.note(
    hasWebSearchKey
      ? [
          ...
          "Docs: https://docs.zee.bot/tools/web",
        ].join("\n")
      : [
  ...
  await prompter.note(
    'What now: https://zee.bot/showcase ("What People Are Building").',
    "What now",
  );
