...
function getBrowserControlToken(): string | null {
  const env = process.env.ZEE_BROWSER_CONTROL_TOKEN?.trim();
  if (env) return env;
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  const hint = `Start (or restart) the Zee gateway (Zee.app menubar, or \`${formatCliCommand("zee gateway")}\`) and try again.`;
