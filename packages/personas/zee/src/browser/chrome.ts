...
export async function launchClawdChrome(
  ...
  if (needsDecorate) {
    try {
      ...
      log.info(`[zee] clawd browser profile decorated (${profile.color})`);
    } catch (err) {
  ...
  log.info(
    `[zee] clawd browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
