...
type Client = {
  ...
  connId: string;
  presenceKey?: string;
  isAlive: boolean; // ping/pong keepalive tracking
};
...
export async function startGatewayServer(
  ...
  // periodic keepalive (application-level tick)
  ...
  // WebSocket-level ping/pong keepalive to detect dead connections
  const pingInterval = setInterval(() => {
    for (const c of clients) {
      if (!c.isAlive) {
        // Client didn't respond to last ping - terminate connection
        logWsControl.warn(
          `ping timeout, closing connection connId=${c.connId}`,
        );
        try {
          c.socket.terminate();
        } catch {
      ...
      // Mark as not alive and send ping; client pong handler will mark alive again
      c.isAlive = false;
      try {
        c.socket.ping();
      } catch {
  ...
  wss.on("connection", (socket, upgradeReq) => {
    ...
    const forwardedForHeader = upgradeReq.headers["x-forwarded-for"];
    const forwardedFor = Array.isArray(forwardedForHeader)
      ? forwardedForHeader.join(",")
      : forwardedForHeader;
    const requestOriginHeader = upgradeReq.headers.origin;
    const requestOrigin = Array.isArray(requestOriginHeader)
      ? requestOriginHeader[0]
      : requestOriginHeader;
    const requestHostHeader = upgradeReq.headers.host;
    const requestHost = Array.isArray(requestHostHeader)
      ? requestHostHeader[0]
      : requestHostHeader;
    const requestUserAgentHeader = upgradeReq.headers["user-agent"];
    const requestUserAgent = Array.isArray(requestUserAgentHeader)
      ? requestUserAgentHeader[0]
    ...
    // ping/pong keepalive: mark client as alive when pong received
    socket.on("pong", () => {
    ...
    socket.on("message", async (data) => {
      ...
      try {
        ...
        const paramsRecord = (req.params ?? {}) as Record<string, unknown>;
        const debug = extractDebugFlag(paramsRecord);
        const payloadPolicy = resolveWideEventPolicy();
        const { summary: paramsSummary, extracted } = summarizeGatewayParams(
          paramsRecord,
          { debug, policy: payloadPolicy },
        );
        const traceId = extractTraceId(paramsRecord);
        const clientInfo = client?.connect?.client;
        const wideEventStartMs = Date.now();
        const wideEventTs = new Date(wideEventStartMs).toISOString();
        const baseWideEvent = {
          ts: wideEventTs,
          service: "zee-gateway",
          traceId,
        ...
        const respond = (
          ...
          if (!areWideEventsEnabled()) return;
          const durationMs = Date.now() - wideEventStartMs;
          void emitWideEvent({
            event: baseWideEvent,
            ok,
            durationMs,
        ...
        void (async () => {
          ...
          const handlerError = formatForLog(err);
          respond(
            false,
            undefined,
            errorShape(ErrorCodes.UNAVAILABLE, handlerError),
            { handlerError },
          );
        });
      } catch (err) {
    ...
    close: async (opts) => {
      ...
      const restartExpectedMs =
        ...
        Number.isFinite(opts.restartExpectedMs)
          ...
          : null;
      // Stop config reloader first to prevent race conditions during shutdown
      ...
      // Stop all providers in parallel with timeout
      const providerStops = [
        stopWhatsAppProvider(),
        stopTelegramProvider(),
        stopDiscordProvider(),
        ...
        Promise.race([
          ...
          new Promise<void>((resolve) =>
            setTimeout(resolve, PROVIDER_SHUTDOWN_TIMEOUT_MS),
          ),
        ]),
      );
      await Promise.allSettled(providerStops);
      cron.stop();
      heartbeatRunner.stop();
