...
export const providersHandlers: GatewayRequestHandlers = {
  "providers.status": async ({ params, respond, context }) => {
    ...
    // Build telegram accounts list - include user account if enabled
    const telegramUserModeEnabled = isTelegramUserModeEnabled(cfg);
    const telegramUserConfig = telegramUserModeEnabled
      ? getTelegramUserConfig(cfg)
      : null;
    const telegramUserAccountId = "user";

    // Get user account status from runtime if it exists
    const userAccountData = telegramUserModeEnabled
      ? (() => {
          const rt = runtime.telegramAccounts?.[telegramUserAccountId];
          return {
            accountId: telegramUserAccountId,
            name: "Zee (User)",
            enabled: true,
            configured: Boolean(telegramUserConfig?.phoneNumber),
            ...
            probe: undefined as TelegramProbe | undefined,
            lastProbeAt: null as number | null,
            phoneNumber: telegramUserConfig?.phoneNumber ?? null,
          };
        })()
      : null;

    const botAccounts = await Promise.all(
      listTelegramAccountIds(cfg).map(async (accountId) => {
        const account = resolveTelegramAccount({ cfg, accountId });
        const configuredDefault = resolveDefaultTelegramAccountId(cfg);
        const rt =
          runtime.telegramAccounts?.[account.accountId] ??
          (account.accountId === configuredDefault
            ? runtime.telegram
            : undefined);
    ...
    // Combine user account (first) with bot accounts
    const telegramAccounts = [
      ...(userAccountData ? [userAccountData] : []),
      ...botAccounts,
    ];

    // Default to user account if in user mode, otherwise configured default
    const defaultTelegramAccountId = telegramUserModeEnabled
