  ...
  send: async ({ params, respond, context }) => {
    ...
    if (!to) {
      respond(
        false,
        undefined,
        errorShape(ErrorCodes.INVALID_REQUEST, "to required"),
      );
      return;
    }
    const message =
    ...
    const mediaUrl =
      typeof request.mediaUrl === "string" ? request.mediaUrl.trim() : "";
    if (!message && !mediaUrl) {
      respond(
        false,
        undefined,
        errorShape(ErrorCodes.INVALID_REQUEST, "message or mediaUrl required"),
      );
      return;
    }
  ...
  poll: async ({ params, respond, context }) => {
    ...
    if (!question) {
      respond(
        false,
        undefined,
        errorShape(ErrorCodes.INVALID_REQUEST, "question required"),
      );
      return;
    }
    ...
    if (options.some((option) => !option)) {
      respond(
        false,
        undefined,
        errorShape(ErrorCodes.INVALID_REQUEST, "options must be non-empty"),
      );
      return;
    }
    const provider = normalizeMessageProvider(request.provider) ?? "whatsapp";
