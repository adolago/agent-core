...
export function createProviderManager(
  ...
  const startTelegramProvider = async (accountId?: string) => {
    ...
    // Start user mode provider if enabled (can run alongside bot mode)
    if (isTelegramUserModeEnabled(cfg)) {
      const userConfig = getTelegramUserConfig(cfg);
      const userAccountId = "user";

      if (!telegramTasks.has(userAccountId)) {
        if (!userConfig.phoneNumber) {
          const latest =
            telegramRuntimes.get(userAccountId) ?? defaultTelegramStatus();
          telegramRuntimes.set(userAccountId, {
            ...latest,
            running: false,
            lastError: "phone number not configured",
          });
          logTelegram.warn(
            "[user] telegram user provider not configured (no phoneNumber)",
          );
        } else {
          // Check if API credentials exist
          const userConfigured = await isTelegramUserConfigured(cfg);
          if (!userConfigured) {
            const latest =
              telegramRuntimes.get(userAccountId) ?? defaultTelegramStatus();
            telegramRuntimes.set(userAccountId, {
              ...latest,
              running: false,
              lastError: "API credentials not found",
            });
            logTelegram.warn(
              "[user] telegram user provider not configured (missing ~/.zee/credentials/telegram/api.json)",
            );
          } else {
            logTelegram.info(
              `[user] starting user provider (${userConfig.phoneNumber})`,
            );
            const abort = new AbortController();
            telegramAborts.set(userAccountId, abort);
            const latest =
              telegramRuntimes.get(userAccountId) ?? defaultTelegramStatus();
            telegramRuntimes.set(userAccountId, {
              ...latest,
              ...
              lastError: null,
              mode: "polling", // User accounts use MTProto polling
            });

            const task = monitorTelegramUserProvider({
              ...
              .catch((err) => {
                const current =
                  telegramRuntimes.get(userAccountId) ??
                ...
                logTelegram.error(
                  `[user] provider exited: ${formatError(err)}`,
                );
              })
              .finally(() => {
                telegramAborts.delete(userAccountId);
                telegramTasks.delete(userAccountId);
                const current =
                  telegramRuntimes.get(userAccountId) ??
                  defaultTelegramStatus();
                telegramRuntimes.set(userAccountId, {
            ...
            telegramTasks.set(userAccountId, task);
          }
        }
      }
    }

    // Bot mode (runs alongside user mode if both are configured)
  ...
  const getRuntimeSnapshot = (): ProviderRuntimeSnapshot => {
    ...
    // Include user mode account if enabled
    const telegramUserAccountId = "user";
    const telegramUserStatus = isTelegramUserModeEnabled(cfg)
      ? (() => {
          const userConfig = getTelegramUserConfig(cfg);
          const current =
            telegramRuntimes.get(telegramUserAccountId) ??
            defaultTelegramStatus();
          return {
            ...current,
            mode: current.mode ?? "polling", // User accounts use MTProto polling
            name: "Zee (User)",
    ...
    const telegramAccounts = Object.fromEntries([
      // Include user account first if enabled
      ...(telegramUserStatus
        ? [[telegramUserAccountId, telegramUserStatus] as const]
        : []),
      // Then include configured bot accounts
      ...listTelegramAccountIds(cfg).map((id) => {
        ...
        return [account.accountId, status] as const;
      }),
    ]);
    // For user mode, use "user" as default; otherwise use configured default
    const telegramDefaultId = isTelegramUserModeEnabled(cfg)
      ? telegramUserAccountId
