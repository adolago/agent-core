...
describe("gateway server chat", () => {
  ...
  test("chat.send accepts attachments without message", async () => {
    const { server, ws } = await startServerWithClient();
    await connectOk(ws);
  ...
  test("chat.send rejects empty message without attachments", async () => {
    const { server, ws } = await startServerWithClient();
    await connectOk(ws);
    ...
    expect(res.error?.message ?? "").toMatch(
      /message or attachments required/i,
    );
