...
export function jidToE164(jid: string): string | null {
  ...
  if (lidMatch) {
    ...
    // Try multiple possible locations for LID mapping files
    const mappingPaths = [
      `${CONFIG_DIR}/credentials/whatsapp/default/lid-mapping-${lid}_reverse.json`,
      `${CONFIG_DIR}/credentials/lid-mapping-${lid}_reverse.json`,
    ];
