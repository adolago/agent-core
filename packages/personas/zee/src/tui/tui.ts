...
function formatAssistantErrorForUi(errorMessage: string): string {
  if (!errorMessage) return "Error: assistant failed";
  if (/^error[:\s]/i.test(errorMessage)) return errorMessage;
    ...
    if (stopReason === "error") {
      const errorMessage =
        typeof record.errorMessage === "string"
          ...
          : "";
      return formatAssistantErrorForUi(errorMessage);
    }
    return "";
  }
  return extractTextBlocks(content, opts);
}
