...
export async function getReplyFromConfig(
  ...
  // Detect if user mentioned a specific persona (@stanley, @johny, etc.)
  const detectedPersona = detectPersonaMention(rawBody);
  ...
  if (resolvedQueue.mode === "interrupt" && laneSize > 0) {
    ...
    const wasActive = isEmbeddedPiRunActive(sessionIdFinal);
    if (wasActive) {
      abortEmbeddedPiRun(sessionIdFinal);
    }
    logVerbose(
      `Interrupting ${sessionLaneKey} (cleared ${cleared}, aborted=${wasActive})`,
    );
  }
