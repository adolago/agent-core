...
function normalizePluginHookEntry(api: ZeePluginApi, entry: HookEntry): HookEntry {
  return {
    ...
    zee: {
      ...entry.zee,
      hookKey: entry.zee?.hookKey ?? `${api.id}:${entry.hook.name}`,
      events: entry.zee?.events ?? [],
    },
  };
}
...
async function loadHookHandler(
  ...
  try {
    ...
    const exportName = entry.zee?.export ?? "default";
...
export async function registerPluginHooksFromDir(
  ...
  for (const entry of hooks) {
    ...
    const events = normalizedEntry.zee?.events ?? [];
    if (events.length === 0) {
