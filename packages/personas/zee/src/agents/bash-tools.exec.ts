...
function isPtyShimEnabled(): boolean {
  return process.env.ZEE_PTY_SHIM === "1" || Boolean(process.env.VITEST);
}

// Test shim: emulate the PTY interface using stdio pipes when node-pty is absent.
function createPtyShimSpawn(): PtySpawn {
  return (file, args, options) => {
    const child = spawn(file, Array.isArray(args) ? args : [args], {
      cwd: options.cwd,
      env: options.env,
      stdio: ["pipe", "pipe", "pipe"],
      windowsHide: true,
    }) as ChildProcessWithoutNullStreams;
    const dataListeners = new Set<PtyListener<string>>();
    const exitListeners = new Set<PtyListener<PtyExitEvent>>();
    ...
    child.once("exit", emitExit);
    child.once("close", emitExit);
    return {
      pid: child.pid ?? 0,
      write: (data) => {
...
async function loadPtySpawn(): Promise<PtySpawn | null> {
  try {
    ...
    if (spawnPty) return spawnPty;
  } catch {
    // ignore import failures
  }
  return isPtyShimEnabled() ? createPtyShimSpawn() : null;
}
