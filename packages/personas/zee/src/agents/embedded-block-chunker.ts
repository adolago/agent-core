...
export class EmbeddedBlockChunker {
  ...
  /**
   * Alias for push() - add text to buffer.
   */
  ...
  /**
   * Drain chunks from the buffer.
   * @param options.force - If true, flush all remaining content
   * @param options.emit - Callback for each emitted chunk
   */
  drain(options: { force: boolean; emit: (chunk: string) => void }): void {
    if (options.force) {
      const remaining = this.flush();
      if (remaining) {
        options.emit(remaining);
      }
      return;
    }
