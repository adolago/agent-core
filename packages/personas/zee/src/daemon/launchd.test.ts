...
async function withLaunchctlStub(
  ...
  const originalLogPath = process.env.ZEE_TEST_LAUNCHCTL_LOG;
  const originalListOutput = process.env.ZEE_TEST_LAUNCHCTL_LIST_OUTPUT;

  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zee-launchctl-test-"));
  try {
    ...
    process.env.ZEE_TEST_LAUNCHCTL_LOG = logPath;
    process.env.ZEE_TEST_LAUNCHCTL_LIST_OUTPUT = options.listOutput ?? "";
    ...
    if (originalLogPath === undefined) {
      delete process.env.ZEE_TEST_LAUNCHCTL_LOG;
    } else {
      process.env.ZEE_TEST_LAUNCHCTL_LOG = originalLogPath;
    }
    if (originalListOutput === undefined) {
      delete process.env.ZEE_TEST_LAUNCHCTL_LIST_OUTPUT;
    } else {
      process.env.ZEE_TEST_LAUNCHCTL_LIST_OUTPUT = originalListOutput;
    }
...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    await withLaunchctlStub({ listOutput: "123 0 bot.zee.gateway\n" }, async ({ env }) => {
...
describe("resolveLaunchAgentPlistPath", () => {
  it("uses default label when ZEE_PROFILE is default", () => {
    const env = { HOME: "/Users/test", ZEE_PROFILE: "default" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/bot.zee.gateway.plist",
    );
  });

  it("uses default label when ZEE_PROFILE is unset", () => {
  ...
  it("uses profile-specific label when ZEE_PROFILE is set to a custom value", () => {
    const env = { HOME: "/Users/test", ZEE_PROFILE: "jbphoenix" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/bot.zee.jbphoenix.plist",
    );
  });

  it("prefers ZEE_LAUNCHD_LABEL over ZEE_PROFILE", () => {
    const env = {
      HOME: "/Users/test",
      ZEE_PROFILE: "jbphoenix",
      ZEE_LAUNCHD_LABEL: "com.custom.label",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
  ...
  it("trims whitespace from ZEE_LAUNCHD_LABEL", () => {
    const env = {
      HOME: "/Users/test",
      ZEE_LAUNCHD_LABEL: "  com.custom.label  ",
    };
  ...
  it("ignores empty ZEE_LAUNCHD_LABEL and falls back to profile", () => {
    const env = {
      HOME: "/Users/test",
      ZEE_PROFILE: "myprofile",
      ZEE_LAUNCHD_LABEL: "   ",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/bot.zee.myprofile.plist",
    );
  });
  ...
  it("trims whitespace from ZEE_PROFILE", () => {
    const env = { HOME: "/Users/test", ZEE_PROFILE: "  myprofile  " };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/bot.zee.myprofile.plist",
    );
  });
