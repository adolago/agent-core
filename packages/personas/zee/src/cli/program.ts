...
import {
  wideEventsCommand,
  wideEventsHealthCommand,
  wideEventsProbeCommand,
} from "./wide-events-cli.js";
...
export function buildProgram(options?: { enableModelsCli?: boolean }) {
  ...
  program
    ...
    .option(
      "--workspace <dir>",
      "Agent workspace directory (default: ~/zee; stored as agent.workspace)",
    )
...
Examples:
  ...
  zee send --to +15555550123 --media photo.jpg
...
Shows token usage per session when the agent reports it; set agent.contextTokens to see % of your model window.`,
  ...
  const wideEvents = program
    .command("wide-events")
    .description("Tail wide event logs");
  wideEvents
    .command("health")
    .description("Check wide event logging health")
    .option("--json", "Output JSON instead of text", false)
    .action(async (opts) => {
      try {
        await wideEventsHealthCommand(
          { json: Boolean(opts.json) },
          defaultRuntime,
        );
      } catch (err) {
  ...
  wideEvents
    .option("--file <path>", "Wide event log file path")
    .option("--lines <count>", "Number of lines to show", "200")
    .option(
      "--where <filter...>",
      "Filter as key=value (repeatable)",
      (val: string, memo: string[]) => memo.concat(val),
      [] as string[],
    )
    .option("--json", "Output JSON instead of text", false)
    .action(async (opts) => {
      try {
        await wideEventsCommand(
          {
            file: opts.file as string | undefined,
            lines: opts.lines as string | number | undefined,
  ...
  addGatewayClientOptions(
    wideEvents
      .command("probe")
      .description("Emit a test wide event via the Gateway"),
  )
    .option("--json", "Output JSON instead of text", false)
    .action(async (opts, cmd) => {
      try {
        const json = Boolean(cmd?.opts?.().json ?? cmd?.parent?.opts?.().json);
        await wideEventsProbeCommand({ ...opts, json }, defaultRuntime);
      } catch (err) {
        defaultRuntime.error(String(err));
        defaultRuntime.exit(1);
      }
