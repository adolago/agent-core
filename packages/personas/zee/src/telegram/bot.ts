...
import { canShowThinking } from "../config/provider-capabilities.js";
import {
  loadSessionStore,
  resolveStorePath,
  updateLastRoute,
...
// Use full Message type to include all possible message properties (text, photo, video, etc.)
...
export function createTelegramBot(opts: TelegramBotOptions) {
  ...
  const processMessage = async (
    ...
    const hasAnyMention = (msg.entities ?? msg.caption_entities ?? []).some(
      (ent: { type: string }) => ent.type === "mention",
    );
    ...
    const { queuedFinal } = await dispatchReplyFromConfig({
      ...
      replyOptions: {
        ...
        onPartialReply: draftStream
          ...
          : undefined,
        // Thinking/reasoning output is never shown on messaging platforms
        onReasoningStream:
          draftStream && canShowThinking("telegram")
            ? (payload) => {
