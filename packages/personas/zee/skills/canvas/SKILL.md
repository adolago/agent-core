...
```
http://<tailscale-hostname>:18793/__zee__/canvas/<file>.html
```
...
Place files in the canvas root directory (default `~/zee/canvas/`):

```bash
cat > ~/zee/canvas/my-game.html << 'HTML'
<!DOCTYPE html>
<html>
...
```bash
cat ~/.zee/zee.json | jq '.gateway.bind'
```

Then construct the URL:
- **loopback**: `http://127.0.0.1:18793/__zee__/canvas/<file>.html`
- **lan/tailnet/auto**: `http://<hostname>:18793/__zee__/canvas/<file>.html`
...
**Example:**
```
canvas action:present node:node-63599bc4-b54d-4392-9048-b97abd58343a target:http://peters-gateway-host-1.sheep-coho.ts.net:18793/__zee__/canvas/snake.html
```
...
**Debug steps:**
1. Check server bind: `cat ~/.zee/zee.json | jq '.gateway.bind'`
2. Check what port canvas is on: `lsof -i :18793`
3. Test URL directly: `curl http://<hostname>:18793/__zee__/canvas/<file>.html`
...
Node is offline. Use `zee nodes list` to find online nodes.
...
The canvas host serves from `/__zee__/canvas/` prefix:

```
http://<host>:18793/__zee__/canvas/index.html  → ~/zee/canvas/index.html
http://<host>:18793/__zee__/canvas/games/snake.html → ~/zee/canvas/games/snake.html
```

The `/__zee__/canvas/` prefix is defined by `CANVAS_HOST_PATH` constant.
