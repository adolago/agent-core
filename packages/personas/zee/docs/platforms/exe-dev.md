...
1) Create VM → install Node 22 → install Zee  
2) Run `zee onboard --install-daemon`  
...
For service runs, persist it in `~/.zee/zee.json`:
...
```bash
systemctl --user status zee-gateway[-<profile>].service
```
