...
- Set `gateway.bind: "tailnet"` in `~/.zee/zee.json` on the gateway host.
...
1) Set up a DNS-SD zone (example `zee.internal.`) on the gateway host and publish `_zee-gw._tcp` records.
...
1) Create `~/zee/canvas/index.html` on the gateway host.
