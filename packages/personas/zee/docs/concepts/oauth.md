...
On macOS, the first read may trigger a Keychain prompt; run `zee models status`
...
When Zee refreshes an Anthropic OAuth token (profile `anthropic:claude-cli`), it **writes the new credentials back** to Claude Code's storage:
...
1. Zee refreshes the token → gets new access token
