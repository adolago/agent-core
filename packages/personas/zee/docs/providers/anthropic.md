---
summary: "Use Anthropic Claude via API keys or Claude Code CLI auth in Zee"
read_when:
...
In Zee you can authenticate with an API key or reuse **Claude Code CLI** credentials
...
zee onboard --auth-choice claude-cli
```
...
- Zee writes `auth.profiles["anthropic:claude-cli"].mode` as `"oauth"` so the profile
