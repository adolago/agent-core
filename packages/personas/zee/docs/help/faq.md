...
- [Quick start and first-run setup](#quick-start-and-firstrun-setup)
  ...
  - [I can't access docs.zee.bot (SSL error). What now?](#i-cant-access-docszee-ssl-error-what-now)
...
Related: [Where things live on disk](/help/faq#where-does-zee-store-its-data),
...
Some Comcast/Xfinity connections incorrectly block `docs.zee.bot` via Xfinity
Advanced Security. Disable it or allowlist `docs.zee.bot`, then retry. More
...
```bash
curl -fsSL --proto '=https' --tlsv1.2 https://zee.bot/install.sh | bash -s -- --beta
```

```bash
curl -fsSL --proto '=https' --tlsv1.2 https://zee.bot/install.sh | bash -s -- --install-method git
```

Windows installer (PowerShell):
https://zee.bot/install.ps1
...
```bash
curl -fsSL https://zee.bot/install.sh | bash -s -- --verbose
```

Beta install with verbose:

```bash
curl -fsSL https://zee.bot/install.sh | bash -s -- --beta --verbose
```

For a hackable (git) install:

```bash
curl -fsSL https://zee.bot/install.sh | bash -s -- --install-method git --verbose
```
...
`claude setup-token` generates a **token string** via the Claude Code CLI (it is not available in the web console). You can run it on **any machine**. If Claude Code CLI credentials are present on the gateway host, Zee can reuse them; otherwise choose **Anthropic token (paste setup-token)** and paste the string. The token is stored as an auth profile for the **anthropic** provider and used like an API key or OAuth profile. More detail: [OAuth](/concepts/oauth).

Zee keeps `auth.profiles["anthropic:claude-cli"].mode` set to `"oauth"` so
...
Yes. Zee can **reuse Claude Code CLI credentials** (OAuth) and also supports **setup-token**. If you have a Claude subscription, we recommend **setup-token** for long‑running setups (requires Claude Pro/Max + the `claude` CLI). You can generate it anywhere and paste it on the gateway host. OAuth reuse is supported, but avoid logging in separately via Zee and Claude Code to prevent token conflicts. See [Anthropic](/providers/anthropic) and [OAuth](/concepts/oauth).
...
Zee supports **OpenAI Code (Codex)** via OAuth or by reusing your Codex CLI login (`~/.codex/auth.json`). The wizard can import the CLI login or run the OAuth flow and will set the default model to `openai-codex/gpt-5.2` when appropriate. See [Model providers](/concepts/model-providers) and [Wizard](/start/wizard).

### Do you support OpenAI subscription auth Codex OAuth

Yes. Zee fully supports **OpenAI Code (Codex) subscription OAuth** and can also reuse an
...
Showcase: https://zee.bot/showcase
...
If the Gateway runs on the same machine as Chrome (default setup), you usually **do not** need `zee browser serve`.
...
- **Local browser automation.** Keep the Gateway on a VPS, but run Chrome locally and relay control
  with the Chrome extension + `zee browser serve`.
...
- **Sync the Claude Code CLI token on the gateway host**
  - Run `zee models status` (it loads + syncs Claude Code CLI credentials).
  - If it still says missing: run `claude setup-token` (or `zee models auth setup-token --provider anthropic`) and retry.
- **If the token was created on another machine**
  - Paste it into the gateway host with `zee models auth paste-token --provider anthropic`.
...
zee models auth order set --provider anthropic anthropic:claude-cli anthropic:default
...
```bash
zee models auth order set --provider anthropic --agent main anthropic:claude-cli
```
...
Profiles also suffix service names (`com.zee.<profile>`, `zee-gateway-<profile>.service`, `Zee Gateway (<profile>)`).
