...
zee browser serve --bind <browser-host> --port 18791 --token <token>
```
...
```bash
export ZEE_BROWSER_CONTROL_TOKEN="<token>"
```
...
- `browser.controlToken` / `ZEE_BROWSER_CONTROL_TOKEN` is **only** for authenticating browser control HTTP requests to `browser.controlUrl`.
...
Recommended:
- Keep `zee browser serve` bound to loopback (`127.0.0.1`) and publish it via Tailscale.
...
Best practice here: keep `zee browser serve` on HTTP and terminate TLS in front.
...
Flow:
- You run a **browser control server** (Gateway on the same machine, or `zee browser serve`).
