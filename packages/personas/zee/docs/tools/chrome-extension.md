...
If you see `!`:
- Make sure the Gateway is running locally (default setup), or run `zee browser serve` on this machine (remote gateway setup).
- Open the extension Options page; it shows whether the relay is reachable.

## Do I need `zee browser serve`?
...
you typically **do not** need `zee browser serve`.

The Gateway’s built-in browser control server will start on `http://127.0.0.1:18791/` and Zee will
auto-start the local relay server on `http://127.0.0.1:18792/`.

### Remote Gateway (Gateway runs elsewhere) — **yes**

If your Gateway runs on another machine, run `zee browser serve` on the machine that runs Chrome
