...
The Gateway can expose a simple TCP bridge for nodes (Android/legacy, macOS node mode), typically on port `18790`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_zee-gateway._tcp` under `~/.zee/dns/` (and `_zee-bridge._tcp` when the bridge is enabled) using the standard discovery domain `zee.internal.`

To make iOS (gateway) and Android (bridge) discover across networks (Vienna ⇄ London), pair this with:
