...
log in with Claude Code CLI on the gateway host, then run `zee models status`
to sync the refreshed token into Zee’s auth store.
...
zee gateway status
zee gateway stop
# Or: launchctl bootout gui/$UID/com.zee.gateway (replace with com.zee.<profile> if needed)
```
