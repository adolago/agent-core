---
summary: "Gateway-owned node pairing (Option B) for bridge nodes"
read_when:
  - Implementing node pairing approvals without macOS UI
  - Adding CLI flows for approving bridge nodes
...
Note: iOS nodes connect directly to the Gateway WebSocket with token/password auth and do not use pairing. This doc covers bridge nodes.

This enables:
- Headless approval via terminal/CLI (no Swift UI required).
- Optional macOS UI approval (Swift app is just a frontend).
- One consistent membership store for Android, macOS node mode, and future bridge nodes.
