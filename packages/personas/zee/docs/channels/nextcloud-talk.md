...
```bash
zee plugins install @zee/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
zee plugins install ./extensions/nextcloud-talk
```
...
2) On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "Zee" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `zee pairing list nextcloud-talk`
  - `zee pairing approve nextcloud-talk <CODE>`
