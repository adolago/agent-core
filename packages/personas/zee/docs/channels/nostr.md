...
read_when:
  - You want Zee to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Zee to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`zee onboard`) and `zee channels add` list optional channel plugins.
...
```bash
zee plugins install @zee/nostr
```

Use a local checkout (dev workflows):

```bash
zee plugins install --link <path-to-zee>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "zee@example.com",
        "lud16": "zee@example.com"
      }
    }
  }
}
