...
Tlon is a decentralized messenger built on Urbit. Zee connects to your Urbit ship and can
...
```bash
zee plugins install @zee/tlon
```

Local checkout (when running from a git repo):

```bash
zee plugins install ./extensions/tlon
```
...
Use these with `zee message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Zee replies in-thread.
