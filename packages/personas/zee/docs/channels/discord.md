...
3. Configure Zee with `channels.discord.token` (or `DISCORD_BOT_TOKEN` as a fallback).
...
7. Direct chats: secure by default via `channels.discord.dm.policy` (default: `"pairing"`). Unknown senders get a pairing code (expires after 1 hour); approve via `zee pairing approve discord <code>`.
...
### Troubleshooting
- First: run `zee doctor` and `zee channels status --probe` (actionable warnings + quick audits).
...
- On startup, Zee resolves channel/user names in allowlists to IDs (when the bot can search members)
...
- Slash commands may still be visible in Discord UI to users who aren’t allowlisted; Zee enforces allowlists on execution and replies “not authorized”.
