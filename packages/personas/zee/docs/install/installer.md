...
read_when:
  - You want to understand `zee.bot/install.sh`
...
Zee ships two installer scripts (served from `zee.bot`):

- `https://zee.bot/install.sh` — “recommended” installer (global npm install by default; can also install from a GitHub checkout)
- `https://zee.bot/install-cli.sh` — non-root-friendly CLI installer (installs into a prefix with its own Node)
 - `https://zee.bot/install.ps1` — Windows PowerShell installer (npm by default; optional git install)
...
```powershell
& ([scriptblock]::Create((iwr -useb https://zee.bot/install.ps1))) -?
```
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=0 curl -fsSL https://zee.bot/install.sh | bash
```
...
```bash
curl -fsSL https://zee.bot/install-cli.sh | bash -s -- --help
```
...
```powershell
iwr -useb https://zee.bot/install.ps1 | iex -InstallMethod git
```

```powershell
iwr -useb https://zee.bot/install.ps1 | iex -InstallMethod git -GitDir "C:\\zee"
```
