...
Default label is `com.zee.gateway` (or `com.zee.<profile>`):

```bash
launchctl bootout gui/$UID/com.zee.gateway
rm -f ~/Library/LaunchAgents/com.zee.gateway.plist
```

If you used a profile, replace the label and plist name with `com.zee.<profile>`.
...
If you used `https://zee.bot/install.sh` or `install.ps1`, the CLI was installed with `npm install -g zee@latest`.
