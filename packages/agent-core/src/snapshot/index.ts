    ...
    for await (const line of $`git -c core.autocrlf=false --git-dir ${git} --work-tree ${Instance.worktree} diff --no-ext-diff --no-renames --numstat ${from} ${to} -- .`
      ...
      const before =
        beforeResult.exitCode === 0
          ? beforeResult.text()
          : beforeResult.stderr.toString().includes("path") &&
            beforeResult.stderr.toString().includes("does not exist")
            ? ""
      ...
      const after =
        afterResult.exitCode === 0
          ? afterResult.text()
          : afterResult.stderr.toString().includes("path") &&
            afterResult.stderr.toString().includes("does not exist")
            ? ""
