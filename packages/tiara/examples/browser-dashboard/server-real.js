...
const resolveStaticPath = (baseDir, requestPath) => {
...
class ClaudeFlowBridgeReal {
    ...
    setupServer() {
        ...
        this.httpServer = http.createServer((req, res) => {
            ...
            } else if (requestPath === '/code' || requestPath === '/code/') {
                relativePath = 'index-code.html';
            } else if (requestPath === '/chat' || requestPath === '/chat/') {
                relativePath = 'index-chat.html';
            } else {
                relativePath = relativePath.replace(/^\/+/, '');
            }

            const filePath = resolveStaticPath(__dirname, relativePath);
            if (!filePath) {
                res.writeHead(403);
                res.end('Forbidden');
                return;
