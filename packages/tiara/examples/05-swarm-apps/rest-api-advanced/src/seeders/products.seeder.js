...
function getSeedPassword(primaryEnvKey, fallbackEnvKey) {
  const envValue =
    process.env[primaryEnvKey] || (fallbackEnvKey ? process.env[fallbackEnvKey] : '');
  if (envValue && envValue.trim() !== '') {
    return envValue;
  }

  const generated = crypto.randomBytes(12).toString('hex');
  const label = fallbackEnvKey ? `${primaryEnvKey} or ${fallbackEnvKey}` : primaryEnvKey;
  console.warn(`WARN: ${label} not set; generated a random password for seed data.`);
  return generated;
}

const adminSeedPassword = getSeedPassword('SEED_ADMIN_PASSWORD', 'SEED_USER_PASSWORD');
