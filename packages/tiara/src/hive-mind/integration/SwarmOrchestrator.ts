...
import { QdrantStore, getQdrantStore, type TaskData, type CommunicationData } from '../core/QdrantStore.js';
...
export class SwarmOrchestrator extends EventEmitter {
  ...
  private async waitForAgentCompletion(
    ...
    return new Promise((resolve, reject) => {
      ...
      const checkCompletion = async () => {
        const agentState = await this.store.getAgent(agent.id);

        if (agentState?.currentTaskId !== taskId) {
          clearTimeout(timer);
          clearInterval(interval);

          // Get task result
          const task = await this.store.getTask(taskId);
          resolve(task?.result ? task.result : {});
        }
      };
  ...
  private async completeTask(task: Task, execution: any): Promise<void> {
    ...
    await this.store.updateTask(task.id, {
      status: 'completed',
      result: finalResult,
      metadata: { ...(await this.store.getTask(task.id))?.metadata, progress: 100 },
    });
  ...
  private async reassignTask(
    ...
    await this.store.reassignTask(taskId, toAgentId);

    // Update agent states
    await this.store.updateAgent(fromAgentId, {
      currentTaskId: undefined,
      status: 'idle',
    });

    await this.store.updateAgent(toAgentId, {
      currentTaskId: taskId,
      status: 'busy',
    });
      ...
      try {
        ...
        for (const [taskId, execution] of this.activeExecutions) {
          ...
          if (task?.status === 'in_progress') {
            ...
            const currentProgress = (task.metadata as any)?.progress ?? 0;

            if (progress !== currentProgress) {
              await this.store.updateTask(taskId, {
                metadata: { ...task.metadata, progress },
              });
