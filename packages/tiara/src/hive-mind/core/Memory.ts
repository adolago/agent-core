...
import { QdrantStore, getQdrantStore, type MemoryData } from './QdrantStore.js';
...
export class Memory extends EventEmitter {
  ...
  async retrieve(key: string, namespace: string = 'default'): Promise<any> {
    ...
    try {
      ...
      if (dbEntry) {
        ...
        const metadata = dbEntry.metadata || {};
  ...
  async retrieveBatch(keys: string[], namespace: string = 'default'): Promise<Map<string, any>> {
    ...
    if (cacheMisses.length > 0) {
      try {
        // This would require implementing batch queries in QdrantStore
    ...
    if (results.length < (options.limit || 10)) {
      ...
      for (const dbEntry of dbResults) {
        const entry: MemoryEntry = {
          ...
          value: dbEntry.value,
          ttl: dbEntry.ttl,
          createdAt: new Date(dbEntry.createdAt ?? Date.now()),
          accessCount: dbEntry.accessCount ?? 0,
          lastAccessedAt: new Date(dbEntry.lastAccessedAt ?? Date.now()),
        };
  ...
  async compress(namespace?: string): Promise<void> {
    const entries = namespace ? await this.list(namespace) : await this.getAllMemoryEntries();
  ...
  private async compressOldEntries(): Promise<void> {
    ...
    for (const entry of oldEntries) {
      const memEntry: MemoryEntry = {
        ...
        lastAccessedAt: new Date(entry.lastAccessedAt ?? Date.now()),
      };
  ...
  private async getAllMemoryEntries(): Promise<MemoryEntry[]> {
    ...
    return entries.map((e) => ({
      ...
      lastAccessedAt: new Date(e.lastAccessedAt ?? Date.now()),
    }));
  }
