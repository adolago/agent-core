...
import { QdrantStore, getQdrantStore, type CommunicationData, type MessagePriority as StorePriority } from './QdrantStore.js';
...
export class Communication extends EventEmitter {
  ...
  async sendMessage(message: Message): Promise<void> {
    ...
    await this.store.createCommunication({
      ...
      toAgentId: message.toAgentId ?? undefined,
      swarmId: this.swarmId,
      messageType: message.type as CommunicationData['messageType'],
      content: JSON.stringify(message.content),
      priority: (message.priority || 'normal') as StorePriority,
  ...
  async getStats(): Promise<CommunicationStats> {
    ...
    const recentMessages = await this.store.getRecentMessages(this.swarmId, 60000); // Last minute
  ...
  async getPendingMessages(agentId: string): Promise<Message[]> {
    const messages = await this.store.getPendingMessages(agentId);
  ...
  async markDelivered(messageId: string): Promise<void> {
    await this.store.markMessageDelivered(messageId);
  }
  ...
  async markRead(messageId: string): Promise<void> {
    await this.store.markMessageRead(messageId);
  }
  ...
  private startStatsCollector(): void {
    setInterval(async () => {
      ...
      await this.store.storePerformanceMetric({
        swarmId: this.swarmId,
        metricType: 'communication_throughput',
        metricValue: this.stats.throughput,
        metadata: {},
      });
      ...
      await this.store.storePerformanceMetric({
        ...
        metricValue: this.stats.avgLatency,
        metadata: {},
      });
    }, 60000); // Every minute
  }
