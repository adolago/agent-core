...
export class Queen extends EventEmitter {
  ...
  async initialize(): Promise<void> {
    ...
    // Create Queen as a special coordinator agent in store
    await this.store.createAgent({
      ...
      status: 'idle',
      metadata: { role: 'queen', mode: this.config.mode },
    });

    // Register Queen with agent-core process registry
    try {
      await this.agentCore.registerProcess({
        id: this.id,
        type: 'queen',
        name: `Queen-${this.config.swarmId.slice(0, 8)}`,
        swarmId: this.config.swarmId,
        capabilities: [
          'strategic_planning',
          'task_allocation',
      ...
      console.warn('Could not register Queen with agent-core:', err);
    }
  ...
  private async makeStrategicDecision(task: Task, analysis: any): Promise<QueenDecision> {
    // Search for similar past decisions from agent-core memory
    let pastDecisions: any[] = [];
    try {
      const searchResults = await this.agentCore.searchSimilarDecisions(task.description, 3);
      pastDecisions = searchResults.map((r) => ({
        content: r.entry.content,
        score: r.score,
        outcome: r.entry.metadata?.extra?.outcome,
      }));
    } catch (err) {
      // Non-fatal: continue without past decisions
    }
  ...
  private async scoreAgentForTask(
    ...
    // Historical performance (from store)
    const performance = await this.store.getAgentPerformance(agent.id);
    if (performance) {
  ...
  private async applyDecision(decision: QueenDecision): Promise<void> {
    ...
    await this.store.updateTask(decision.taskId, {
      assignedAgents: decision.selectedAgents,
      status: 'assigned',
    });
    ...
    const taskDescription = task?.description ?? 'Unknown task';

    // Store decision in agent-core memory for semantic search
    try {
      await this.agentCore.storeDecision({
        decisionType: 'task_assignment',
        context: taskDescription,
        decision: `Assigned to agents: ${decision.selectedAgents.join(', ')}`,
        rationale: decision.rationale,
        outcome: 'pending',
        swarmId: this.config.swarmId,
        agentIds: decision.selectedAgents,
      });
    } catch (err) {
      console.warn('Could not store decision in agent-core:', err);
    }

    // Also store in MCP for backwards compatibility
  ...
  private async checkTaskProgress(): Promise<void> {
    const activeTasks = await this.store.getActiveTasks(this.config.swarmId);
  ...
  private async checkRebalancing(): Promise<void> {
    const stats = await this.store.getSwarmStats(this.config.swarmId);
  ...
  private async optimizeStrategies(): Promise<void> {
    ...
    const strategyPerformance = await this.store.getStrategyPerformance(this.config.swarmId);
  ...
  private async trainNeuralPatterns(): Promise<void> {
    ...
    const successfulDecisions = await this.store.getSuccessfulDecisions(this.config.swarmId);
  ...
  private async handleAgentFailure(agent: Agent): Promise<void> {
    ...
    await this.store.updateAgentStatus(agent.id, 'offline');
  ...
  private isTaskStalled(task: any): boolean {
    ...
    const lastUpdate = task.metadata?.lastProgressUpdate ?? task.createdAt;
    return lastUpdate && Date.now() - lastUpdate > stalledThreshold;
  }
  ...
  private async reassignTask(taskId: string, fromAgentId: string): Promise<void> {
    ...
    if (availableAgents.length > 0) {
      ...
      await this.store.reassignTask(taskId, newAgent.id);
  ...
  async shutdown(): Promise<void> {
    ...
    try {
      ...
      console.warn('Could not deregister Queen from agent-core:', err);
    }
