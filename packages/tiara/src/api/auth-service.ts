...
export class AuthService {
  ...
  // Optimizes API key lookup from O(N) to O(1) to prevent DoS via CPU exhaustion
  ...
  async authenticateApiKey(apiKey: string): Promise<{ key: ApiKey; user?: User }> {
    try {
      ...
      // Find matching API key using O(1) lookup
      // Since keyHash is derived from input, we can look it up directly.
      // The hash itself acts as a capability.
      const storedKey = this.apiKeysByHash.get(keyHash);
  ...
  async createApiKey(userId: string, keyData: {
    ...
    // Store in hash index for O(1) lookup
    this.apiKeysByHash.set(keyHash, apiKey);
  ...
  private constantTimeCompare(a: string, b: string): boolean {
    ...
    // Use Buffer.from(str) to handle both hex strings (password hashes)
    // and base64url strings (JWT signatures) correctly.
    // Using 'hex' encoding causes invalid parsing for base64url strings,
    // potentially leading to empty buffers and signature bypass.
