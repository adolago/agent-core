...
export class ConfigValidator {
  ...
  async validateMemoryConfig() {
    ...
    try {
      const configPath = `${this.workingDir}/claude-flow.config.json`;
      ...
      if (memoryConfig.backend && memoryConfig.backend !== 'qdrant') {
        result.warnings.push(
          `Memory backend is '${memoryConfig.backend}', expected 'qdrant' for Qdrant persistence`,
        );
      }

      if (!memoryConfig.url) {
        result.warnings.push('Memory config missing memory.url (defaults to http://localhost:6333)');
      }

      if (!memoryConfig.collection) {
        result.warnings.push(
          'Memory config missing memory.collection (defaults to agent_memory)',
        );
      }

      if (!memoryConfig.persistenceCollection) {
        result.warnings.push(
          'Memory config missing memory.persistenceCollection (defaults to agent_persistence)',
        );
      }
    } catch (error) {
      result.warnings.push(
        'claude-flow.config.json not found; Qdrant memory defaults will be used',
      );
    }
