...
export class HealthChecker {
  ...
  async checkMemoryConsistency() {
    ...
    try {
      ...
      // Check config for Qdrant settings if present
      try {
        const configPath = `${this.workingDir}/claude-flow.config.json`;
        const content = await fs.readFile(configPath, 'utf8');
        const config = JSON.parse(content);
        if (config?.memory?.backend && config.memory.backend !== 'qdrant') {
          result.consistent = false;
          result.issues.push(`Memory backend is ${config.memory.backend}, expected qdrant`);
        }
      } catch {
        result.issues.push('claude-flow.config.json not found; using Qdrant defaults');
      }
    } catch {
      result.consistent = false;
      result.issues.push('Cannot validate memory directories');
    }
