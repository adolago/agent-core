...
async function showConfig(subArgs, flags) {
  ...
  try {
    ...
    if (format === 'json') {
      ...
      console.log('\n💾 Memory:');
      console.log(`   Backend: ${config.memory?.backend || 'qdrant'}`);
      console.log(`   URL: ${config.memory?.url || 'http://localhost:6333'}`);
      console.log(`   Collection: ${config.memory?.collection || 'agent_memory'}`);
      console.log(`   Persistence Collection: ${config.memory?.persistenceCollection || 'agent_persistence'}`);
      console.log('\n🤖 Agents:');
...
function showConfigHelp() {
  ...
  console.log('  memory.url                       Qdrant URL');
  console.log('  memory.collection                Qdrant collection');
  console.log('  memory.persistenceCollection     Qdrant persistence collection');
