...
import { printSuccess, printError, printWarning, readJsonFile } from '../utils.js';
import os from 'os';
...
let qdrantStorePromise = null;

async function getQdrantStore() {
  if (!qdrantStorePromise) {
    qdrantStorePromise = (async () => {
...
async function getResourceMetrics() {
  try {
    // Count memory entries from Qdrant
    let memoryEntries = 0;
    try {
      const store = await getQdrantStore();
      memoryEntries = await store.count({ namespace: null });
    } catch {
      memoryEntries = 0;
    }
