...
export async function memoryConsolidationCommand(subArgs, flags) {
  ...
  if (action === 'plan' || action === 'execute') {
    printWarning('Memory consolidation into SQLite is deprecated in Qdrant-only mode.');
    console.log('Use `memory export` to dump legacy data and `memory import` to load into Qdrant.');
    return;
  }
...
function showConsolidationHelp() {
  ...
  console.log('  plan       (deprecated) Use memory export/import for Qdrant migration');
  console.log('  execute    (deprecated) Use memory export/import for Qdrant migration');
  console.log('  report     Generate a consolidation report');
  console.log();
  console.log('Note: Tiara uses Qdrant for memory; consolidation into SQLite is no longer supported.');
  console.log('Use `memory export` to dump legacy data and `memory import` to load into Qdrant.');
  console.log();
