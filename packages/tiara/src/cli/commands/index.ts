...
let persistence: QdrantPersistenceManager | null = null;

async function getPersistence(): Promise<QdrantPersistenceManager> {
  if (!persistence) {
    const config = await getConfigManager();
    const memoryConfig = config.get()?.memory;
    persistence = new QdrantPersistenceManager({
      collection: memoryConfig?.persistenceCollection,
  ...
  cli.command({
    ...
    action: async (ctx: CommandContext) => {
      try {
        ...
        // Ensure memory directory exists for session files
        ...
        console.log('  ✓ Persistence configured for Qdrant (no local database created)');
        ...
        console.log('\nNote: Persistence data is stored in Qdrant');
      } catch (err) {
...
## Quick Reference
- Project uses Qdrant for memory persistence
- Memory is organized by namespaces
- Query with \`npx claude-flow memory query <search>\`

## Storage Location
- Qdrant URL: \`http://localhost:6333\`
- Memory Collection: \`agent_memory\`
- Persistence Collection: \`agent_persistence\`
...
## Overview
The Claude-Flow memory system provides persistent storage and intelligent retrieval of information across agent sessions using Qdrant.

## Storage Backends
- **Primary**: Qdrant collection (\`agent_memory\`)
- **Persistence**: Qdrant collection (\`agent_persistence\`)
