...
export class NeuralInitCommand {
  ...
  async execute(options: InitOptions = {}): Promise<void> {
    const absoluteTarget = this.resolveTargetDir(options.targetDir, '.claude/agents/neural');
    ...
    try {
      ...
      if (verified) {
        logger.info(chalk.green('✅ Neural module initialized successfully!'));
        this.printUsage();
      } else {
  ...
  private resolveTargetDir(targetDir: string | undefined, defaultTargetDir: string): string {
    const baseDir = process.cwd();
    const resolvedTarget = path.resolve(baseDir, targetDir || defaultTargetDir);
    const relativeTarget = path.relative(baseDir, resolvedTarget);
