 ...
 * Includes sliding window and token bucket algorithms.
 *
 * Ported from claude-flow v3 @claude-flow/shared/resilience
 *
 * @module tiara/resilience/rate-limiter
 */
...
export class SlidingWindowRateLimiter implements RateLimiter {
   ...
   * Consume a request token
   */
  consume(key = "default"): RateLimitResult {
   ...
   * Get current status
   */
  status(key = "default"): RateLimitResult {
...
/**
 * Express-like request type for middleware
 */
...
/**
 * Express-like response type for middleware
 */
...
export function createRateLimiterMiddleware(
  limiter: RateLimiter,
): (req: RateLimiterRequest, res: RateLimiterResponse, next: () => void) => void {
  return (req, res, next) => {
