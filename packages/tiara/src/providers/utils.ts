...
export function getDefaultProviderConfig(): ProviderManagerConfig {
  ...
  return {
    ...
    providers: {
      ...
      'llama-cpp': {
        provider: 'llama-cpp',
        apiUrl: process.env.LLAMA_CPP_API_URL,
        model: 'llama-2-7b',
        temperature: 0.7,
        maxTokens: 2048,
...
function loadProviderConfigs(
  ...
  for (const [provider, config] of Object.entries(loaded)) {
    ...
    const temperatureValue = process.env[`${envPrefix}TEMPERATURE`];
    if (temperatureValue) {
      config.temperature = parseFloat(temperatureValue);
    }
    const maxTokensValue = process.env[`${envPrefix}MAX_TOKENS`];
    if (maxTokensValue) {
      config.maxTokens = parseInt(maxTokensValue, 10);
    }
