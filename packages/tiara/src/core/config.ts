...
export class ConfigManager {
  ...
  private setupValidationRules(): void {
    ...
    this.validationRules.set('memory.url', {
      type: 'string',
    });

    this.validationRules.set('memory.collection', {
      type: 'string',
    });

    this.validationRules.set('memory.persistenceCollection', {
      type: 'string',
    });

    this.validationRules.set('memory.apiKey', {
      type: 'string',
    });

    this.validationRules.set('memory.namespace', {
      type: 'string',
    });
  ...
  getSchema(): any {
    return {
      ...
      memory: {
        backend: { type: 'string', values: ['qdrant'] },
  ...
  private loadFromEnv(): Partial<Config> {
    ...
    const memoryCollection =
      process.env.MEMORY_QDRANT_COLLECTION || process.env.QDRANT_COLLECTION_MEMORY;
    const persistenceCollection =
      process.env.MEMORY_QDRANT_PERSISTENCE_COLLECTION ||
      process.env.QDRANT_COLLECTION_PERSISTENCE;
    const memoryApiKey = process.env.MEMORY_QDRANT_API_KEY || process.env.QDRANT_API_KEY;

    if (memoryUrl || memoryCollection || persistenceCollection || memoryApiKey) {
      config.memory = {
        ...DEFAULT_CONFIG.memory,
        ...config.memory,
