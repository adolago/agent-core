...
export function formatBytes(bytes: number, decimals = 2): string {
  ...
  // Math.log2(1024) is 10
  const i = Math.floor(Math.log2(absBytes) / 10);
  const index = i < 0 ? 0 : (i >= BYTES_SIZES.length ? BYTES_SIZES.length - 1 : i);
  ...
  const sign = bytes < 0 ? '-' : '';
  return sign + rounded + ' ' + BYTES_SIZES[index];
}
