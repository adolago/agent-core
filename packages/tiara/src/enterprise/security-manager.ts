...
export class SecurityManager extends EventEmitter {
  ...
  private getDefaultScanner(type: SecurityScan['type']): string {
    const scanners: Partial<Record<SecurityScan['type'], string>> = {
      vulnerability: 'trivy',
    ...
    const secretsScanner = this.getConfiguredSecretsScanner();
    if (secretsScanner) {
      scanners.secrets = secretsScanner;
    }

    return scanners[type] || 'generic';
  }

  private getConfiguredSecretsScanner(): string | undefined {
    const envValue = process.env.CLAUDE_FLOW_SECRETS_SCANNER;
