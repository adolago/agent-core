...
This directory documents the in-repo hosted platform for agent-core. The hosted service lives in `packages/hosted` and provides the share API, public share viewer, hosted console, provider vault, analytics, billing surfaces, gateway routing, and telemetry/log ingestion.
...
- Public viewer: `GET /share/:slug` or `GET /s/:slug`
- Console: `GET /console` (login required)
- Org/workspace auth and API keys
...
The hosted console uses local email + password accounts. The first account can be bootstrapped by setting:
...
Visit `http://127.0.0.1:8787/console` to manage orgs, workspaces, API keys, providers, and retention settings.
