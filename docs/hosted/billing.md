...
Plans can be updated per workspace through the console or the API.
...
The console will redirect users to the configured portal URL. The hosted service stores the plan selection and entitlement limits in the database; external billing systems can synchronize plan changes by calling `POST /api/workspaces/:workspaceId/plan`.
