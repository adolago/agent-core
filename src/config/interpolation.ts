...
/**
 * Non-global patterns for stateless .test() operations
 * Using global regex with .test() causes lastIndex state bugs
 */
const ENV_PATTERN_TEST = /\{env:([^}]+)\}/;
const FILE_PATTERN_TEST = /\{file:([^}]+)\}/;
...
/**
 * Check if a position in text is inside a comment.
 * Handles:
 * - Single-line comments (//)
 * - Multi-line comments (/* *\/)
 * - Comments at end of lines
 * - Ignores comment-like patterns inside string literals
 */
function isInComment(text: string, position: number): boolean {
  ...
  while (i < position) {
    ...
    // Skip comment detection if inside a string
    if (inString) {
      i++;
      continue;
    }
    ...
    // Don't start new comments if already in one
    if (!inSingleLineComment && !inMultiLineComment) {
...
export async function interpolate(
  ...
  for (const match of fileMatches) {
    ...
    // Skip if match is inside a comment (JSONC support)
    if (isInComment(protectedText, matchIndex)) {
      continue;
    }
 ...
 * Uses non-global patterns to avoid stateful lastIndex issues
 */
export function hasInterpolation(text: string): boolean {
  return ENV_PATTERN_TEST.test(text) || FILE_PATTERN_TEST.test(text);
}
