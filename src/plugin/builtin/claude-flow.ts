 ...
 *
 * Integrates claude-flow hooks system for task coordination,
...
export const ClaudeFlowPlugin: PluginFactory = async (
  ...
  async function executeHook(
    ...
    try {
      return await ctx.shell(`npx claude-flow@alpha hooks ${hookType} ${argsStr}`);
    } catch (error) {
  ...
  return {
    metadata: {
      ...
      tags: ['hooks', 'claude-flow', 'coordination'],
    },
    ...
    hooks: {
      ...
      'session.start': async (input, output) => {
        ...
        if (result.exitCode === 0) {
          ctx.logger.debug('Restored session from claude-flow', {
            sessionId: currentSessionId,
          });
        }
      ...
      'post-task': async (input, output) => {
        ...
        if (config.neuralTraining && input.success) {
          try {
            await ctx.shell(
              `npx claude-flow@alpha neural train --pattern coordination --data "${input.taskId}:${input.duration}ms"`
            );
          } catch {
      ...
      'memory.update': async (input, output) => {
        ...
        // Sync memory update to claude-flow
